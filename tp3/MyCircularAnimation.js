class MyCircularAnimation{
    constructor(scene, from, to , duration){
        ...
        this.yRotation = this.calculateYRotation();
    ...
    update(deltaTime){
        ...
        this.circlePoint = [
            this.center[0] + this.radius * Math.cos(rotationAngle) * Math.cos(this.yRotation),
            this.center[1] + this.radius * Math.sin(rotationAngle), 
            this.center[2] + this.radius * Math.cos(rotationAngle) * Math.sin(this.yRotation)
        ]
    ...
    calculateYRotation(){
        ...
        let centerToFrom = [this.from[0] - this.center[0], this.from[1] - this.center[1], this.from[2] - this.center[2]]
        let centerPlusX = [1,0,0]

        //Calculate dot product between centerToFrom and centerPlusX
        let dotProduct = centerToFrom[0] * centerPlusX[0] + centerToFrom[1] * centerPlusX[1] + centerToFrom[2] * centerPlusX[2]
        //Calculate magnitude of centerToFrom and centerPlusX
        let magnitude = Math.sqrt(Math.pow(centerToFrom[0], 2) + Math.pow(centerToFrom[1], 2) + Math.pow(centerToFrom[2], 2)) * Math.sqrt(Math.pow(centerPlusX[0], 2) + Math.pow(centerPlusX[1], 2) + Math.pow(centerPlusX[2], 2))
